...
(function($) {
    ...
    DataTable.DEFAULTS = {
        ...
        //index column
        indexable:true, //enable index
        indexstart:1       //index start from
    };
    ...
    DataTable.prototype.render = function() {
        ...
        for(i = 0; i < cols.length; i++) {
            ...
            if (i === 0 && options.indexable) {
                $tr.append('<th class="sort-disabled text-center" style="width:50px;">#</th>');
            }
            if(i === 0 && checkable) {
        ...
        for(var r = 0; r < rowLen; ++r) {
            ...
            rowColLen =row.data? row.data.length: Object.getOwnPropertyNames(row).length-1;
            for(i = 0; i < rowColLen; ++i) {
                if(!cols[i])continue;

                rowCol = row.data? row.data[i]:row;
                if(i > 0 && rowCol.empty) {
                    continue;
                }
                ...
                if (i === 0 && options.indexable) {
                    $tr.append($('<td class="text-center" style="width:50px">'+(options.indexstart+row.rowid)+'</td>'));
                }
                if(i === 0 && checkable) {
                    $cTd = $('<td data-index="check" class="check-row check-btn"><i class="icon-check-empty"></i></td>');
                    if(options.checkboxName) {
                        $cTd.append('<input class="hide" type="checkbox" name="' + options.checkboxName + '" value="' + row.rowid + '">');
                    }
                    $tr.append($cTd);
                }
                ...
                $tr.append($td);
                $.each(cols,function(_i,_col){
                    if(_col.dataable){
                          $tr.attr("data-field-"+_col.field,(row.data?(typeof row.data[_i]==="object"?row.data[_i].text:row.data[_i]):row[_col.field]))
                    }
                })
            }
    ...
    DataTable.prototype.sortTable = function($th) {
        ...
        //     cellA =cellA.data? cellA.data[index]:cellA;
        //     cellB = cellB.data?cellB.data[index]:cellB;
...
(function ($) {
    ...
    Grid.DEFAULTS = {
        //index column
       indexenable:true, //able index
       indexstart:1,       //index start from
       //size setting
       sizeable:true,
       sizes:[10,20,50],    //分页条数枚举设置
       //toolbox        
       toolboxable:true, //是否启用工具栏
       btns:[{id:'add', text:'添加',icon:'plus'},
            {id:'edit',text:'修改',icon:'edit',hr:true},
            {id:'remove',text:'删除',icon:'remove text-danger'}],             //按钮组  默认有add,edit,remove
       searchable:true, //是否启用搜索框
       searchfield:null,  //搜索框绑定字段 eg：field1|field2
       searchplaceholder:'关键字搜索',
       searchType:"like",  //搜索框绑定字段类型 like|equle
       _searchValue:null,  //搜索框绑定字段 
       //page setting
       pagerable:true,  //是否分页
       currentpage:0,       //当前页
       currentsize:10,      //当前页显示条数
       total:0,          //数据总数
       pagecount:0,      //页总数
       pagerfit:true,   //分页控件是否固定容器底部
       //data
       datatype:'ajax',//数据来源方式 ajax | list
       url:null,        //数据来源方式为 ajax 时必填
       ajaxtype:'get', //ajax 请求类型 get|post
       data:[],         //数据来源方式为 list 时有用
       cols:[],         //grid 显示列配置
       //datatable setting
       fixedLeftWidth: '50%', 
       ...
       sortable: true,
       sortField:'',
       sortType:'down',   //down|up
       indexable:true,
       
       autoload:true,
       isfirstload:true,
       //template
       btntmpl:'<button id="{{id}}" class="btn btn-link {{cls}}"><i class="icon icon-{{icon}}"></i>{{text}}</button>',
       pagerinfotmpl:'每页 {{pagesize}} 条,共 {{total}} 条',
       searchtmpl:'<div class="input-group" style="width:250px;"><input id="txt_SearchBox" type="text" class="form-control pull-right" placeholder="{{searchplaceholder}}"><span class="input-group-btn"><button id="btnSearch" class="btn btn-default pull-right" type="button">搜索</button></span></div>',
       pagerbtnstmpl:'<ul class="pager pull-left"><li><a id="pager_first" title="首页"><i class="icon icon-step-backward"></i></a></li><li><a id="pager_previous" title="上一页"><i class="icon icon-play icon-flip-horizontal"></i></a></li><li><a title="当前页码" class="currentPage"><input  type="text" value="{{currentpage}}"> / <lable>{{pagecount}}</lable></a></li><li><a id="pager_next" title="下一页"><i class="icon icon-play"></i></a></li><li><a id="pager_last" title="尾页"><i class="icon icon-step-forward"></i></a></li></ul>'
    };

    // Get options
    Grid.prototype.getOptions = function (options) {
        var $e = this.$;
        options = $.extend({}, Grid.DEFAULTS, this.$.data(), options);
        this.options = options;
    };
    ...
    Grid.prototype.load = function (data) {
        ...
        if(this.options.datatype==='ajax'){
            if(!this.options.url)
              throw new Error('no exist grid ajax url!');
              //request paras format '{"pageIndex":1,"pageSize": 2,"sortField":"","sortOrder":"","paras": [{"key":"","value":"","type":""}]}'
            if(this.options.isfirstload && !this.options.autoload){
                _this.render();
                return;
            }
            this.options.indexstart= (this.options.currentpage*this.options.currentsize)+1
         
            var pageparams={
                pageIndex:this.options.currentpage+1,
                pageSize:this.options.currentsize
            };
            if(_this.options.sortable && _this.options.sortField.length>0){
                pageparams.sortfield=_this.options.sortField;
                pageparams.sortorder=_this.options.sortType=='down'?'asc':'desc';
            }

            if(_this.options.searchable==true && _this.options.searchfield)
            {
                pageparams.paras=[];
                var txtsearch=_this.$.find('#txt_SearchBox').val();
                _this.options._searchValue=txtsearch;
                if(txtsearch){
                    _this.options.searchfield.split('|').forEach(function(val,index,arr){
                        pageparams.paras.push({key:val,value:txtsearch,type:_this.options.searchType,filter:'or'});     
                    })
                }
            }

            if(typeof data==='object'){
                data= $.extend({},pageparams,this.$.data(),data)
            }else{
                data= pageparams;
            }
            ...
            $.ajax({
                ...
                data:data,
                success:function(rdata){//response data format  {status:0,data:{pageSize:10,pageindex:1,totalCount=1,data=[{}]}}
                    if(rdata.status!='0'){
                        alert('Error!')
                        return;
                    };
                    _this.options.data=rdata.data.data;
                    // _this.options.currentpage=data.pageindex;
                    // _this.options.currentsize=data.pagesize;
                    _this.options.total=rdata.data.totalCount;
                    _this.options.pagecount=Math.ceil(_this.options.total/_this.options.currentsize);
                    
                    _this.callEvent('afterLoad', {
            ...
            if(data){
                ...
                this.options.total=data.length;
    ...
    Grid.prototype.render = function () {
        ...
        //grid toolbox
        var $toolbox;
        if(options.toolboxable){
            $toolbox=$('<div class="toolbox"></div>');
            if(options.btns.length>0){
                var $btnbox = $('<div class="pull-left"></div>');
                var btntmpl=options.btntmpl;
                $.each(options.btns,function(index,row){
                    $btnbox.append(btntmpl.replace('{{cls}}',(row.cls?row.cls:''))
                        .replace('{{icon}}',(row.icon?row.icon:''))
                        .replace('{{id}}',(row.id?row.id:'btn'+$.zui.uuid()))
                        .replace('{{text}}',(row.text?row.text:'')))
                    if(row.hr){
                         $btnbox.append('<i class="icon icon-ellipsis-v"></i>');
                    }
                }) 
                $toolbox.append($btnbox);
            }
            if(options.searchable){
                var $searchbox=$('<div class="pull-right"></div>');
                $searchbox.append(options.searchtmpl.replace('{{searchplaceholder}}',options.searchplaceholder));
                
                if(options._searchValue)
                    $searchbox.find('#txt_SearchBox').val(options._searchValue)    
                if(!options.searchfield)
                    $searchbox.find('#txt_SearchBox').remove();
                $toolbox.append($searchbox);
            }
            $grid.append($toolbox);    
        }

        // grid content init for datatable
        var $content=$('<div class="grid-content"><div class="datatable"></div></div>');
        if(options.pagerfit)$content.addClass('grid-content-fit');
        $grid.append($content);

        // grid pager
        var $pager;
         if(options.pagerable){
            $pager=$('<div class="grid-footer grid-border-top "></div>');
            $pager.append(options.pagerbtnstmpl
            .replace('{{pagecount}}',options.pagecount)
            .replace('{{currentpage}}',parseInt(options.currentpage)+1))
            .find('.currentPage input').attr('id','index_'+this.id) ;
            var $pagerInfo = $('<label class="pager pull-right" style="margin-top:7px;"></label>');
            var pagerinfotmpl=options.pagerinfotmpl.replace('{{total}}',options.total);
            if(options.sizeable){
                var $sizeSelector = $('<select></select>');
                 $sizeSelector.attr('id','size_'+this.id)
                $.each(options.sizes,function(index,row){
                    $sizeSelector.append('<option value="'+row+'" '+(options.currentsize==row?'selected':'')+'>'+row+'</option>');
                })
                pagerinfotmpl=pagerinfotmpl.replace('{{pagesize}}',$sizeSelector[0].outerHTML)
            }else{
                pagerinfotmpl=pagerinfotmpl.replace('{{pagesize}}',options.currentsize)
            }
            $pagerInfo.append(pagerinfotmpl);
            $pager.append($pagerInfo);
            $grid.append($pager);
        }

        this.$.html($grid);
        this.datatable=this.$.find('.datatable').datatable({
               fixedLeftWidth: options.fixedLeftWidth, 
                fixedRightWidth: options.fixedRightWidth, 
                data: {rows:options.data,cols:options.cols}, 
                fixedHeaderOffset:options.fixedHeaderOffset, 
                ...
                sort:function(e){
                    // console.log({sortfield:options.sortField,filed:e.sorter.field,sorttype:options.sortType,type:e.sorter.type})
                    if( options.sortField==e.sorter.field && options.sortType==e.sorter.type)return;
                    options.sortField=e.sorter.field;
                    options.sortType=e.sorter.type;
                    _this.load();
                },reader:function(e){
    ...
    Grid.prototype.resetpager=function(){
        var $pager=this.$.find('.pager');
        $pager.find('.disabled').removeClass('disabled');
        $pager.find('.input').removeAttr('readonly');
        if(this.options.pagecount<=1){
           $pager.find('a').addClass('disabled');
           $pager.find('input').attr('readonly','readonly');
           return;
        }
      
      var currentPage = parseInt(this.options.currentpage+1);
      if(currentPage===this.options.pagecount){
          $pager.find('#pager_last').addClass('disabled');
          $pager.find('#pager_next').addClass('disabled');
          return;
      }
      
     if(currentPage==1){
        $pager.find('#pager_first').addClass('disabled');
        $pager.find('#pager_previous').addClass('disabled');
        return;
      }
    ...
    Grid.prototype.previous = function () {
         this.goto(this.options.currentpage);
    }

    Grid.prototype.next = function () {
        this.goto(this.options.currentpage+2);
    }
    ...
    Grid.prototype.last = function () {
        this.goto(this.options.pagecount);
    }

    Grid.prototype.goto = function (pageindex) {
        if(pageindex<1)pageindex=1;
        if(pageindex>this.options.pagecount)pageindex=this.options.pagecount
        if(this.options.currentpage==pageindex-1)return;
         this.options.currentpage=pageindex-1;
            //reload data
         this.load();
    }
    ...
    Grid.prototype.bindEvents = function () {
        var _this = this;
        //page size change
        this.$.find('#size_'+this.id).on('change',function(e){
            //setting pagesize
            _this.options.currentsize=parseInt(e.currentTarget.value);
            _this.options.currentpage=0;
            // //reload data
            _this.load();
        });

        //page index change
        this.$.find('#index_'+this.id).on('change',function(e){
            if(_this.options.currentpage==parseInt(e.currentTarget.value)-1){
                e.currentTarget.value=parseInt(e.currentTarget.value);
                return;
            }
            _this.goto(parseInt(e.currentTarget.value))
        });

        this.$.find('#pager_first').on('click',function(e){
            _this.first();
        });

        this.$.find('#pager_last').on('click',function(e){
            _this.last();
        });

        this.$.find('#pager_previous').on('click',function(e){
            _this.previous();
        });

        this.$.find('#pager_next').on('click',function(e){
            _this.next();
        });

        this.$.find('#btnSearch').on('click',function(e){
            _this.load();
        });
    }
    ...
    // Call event
    Grid.prototype.callEvent = function (name, params) {
        var result = this.$.callEvent(name + '.' + this.name, params, this).result;
        return !(result !== undefined && (!result));
    };

    $.fn.grid = function (option, newData) {
        return this.each(function () {
            var $this = $(this);
            var data = $this.data(name);
            var options = typeof option == 'object' && option;

            if (!data) $this.data(name, (data = new Grid(this, options)));
