...
(function ($) {
        ...
        initTree: function (options) {
            ...
            return this.each(function () {
                ...
                if (options && options.data != null) {
                    var $cols = $('<tr>');
                    options.columns.forEach(function (e) {
                        if(e.ignore===true)return;
                        var $col = $('<th>').html(e.text);
                        $cols.append($col);
                    }, this)
                    ...
                    options.data.forEach(function (row) {
                        ...
                        options.columns.forEach(function (col) {
                            ...
                            txt =txt ==null?'':txt;
                            if(col.dataable===true){
                                $row.attr('data-'+col.field,txt);
                            }

                            if(col.ignore===true)return;
                            var _pid = row[options.parentField];
                            var $col = $('<td>').html((options.textField==col.field && _pid>0? '<i class="icon icon-level-up icon-flip-horizontal">':' ')+txt);
                            $row.addClass('treegrid-'+row[options.idField]);
                            if(_pid && _pid>0){
                                $row.addClass('treegrid-parent-'+_pid);
                            }
                            $row.append($col);
                        }, this)
    ...
    $.fn.treegrid.defaults = {
        ...
        treeColumn: 0,
        columns: [],//{field: 'Name', width: 'auto', text: '用户名', colClass: '' , dataable: true }
        data: null,
