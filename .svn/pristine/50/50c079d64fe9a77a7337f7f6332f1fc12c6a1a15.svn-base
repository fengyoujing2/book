    ...
    function loadColumns() {
        ...
        if (that.options.index) {
            that.columns.push({
                ...
                visibleInSelection: false, // default: true
                width: '30px'
            });
        }
        /*jshint -W018*/
    ...
    function renderToolbox() {
        if (this.options.navigation !== 0) {
            var css = this.options.css,
                selector = getCssSelector(css.toolbox),
                toolItems = findFooterAndHeaderItems.call(this, selector);

            var that = this,
                tpl = this.options.templates,
                toolbox = $(tpl.toolbox.resolve(getParams.call(this)));

            var tools = '';
            for (var i = 0; i < this.options.btns.length; i++) {
                var current = this.options.btns[i];
                tools += '<button class="' + (current.style == undefined || current.style == '' ? 'btn btn-default' : current.style) + '" ' + (current.click == undefined || current.click == '' ? '' : 'onclick="' + current.click + '"') + '><i class="' + (current.icon == '' ? '' : current.icon) + '"></i> ' + current.text + '</button>'
            }
            $(toolbox).html(tools);
            replacePlaceHolder.call(this, toolItems, toolbox);
        }
    }
...
debugger;
    ...
    Grid.defaults = {
         ...
         **/
        css: {
            toolbox: "toolbox btn-group pull-left", // must be a unique class name or constellation of class names within the header and footer
        ...
        labels: {
            all: "全部",
            infos: "当前 {{ctx.start}}-{{ctx.end}}  共 {{ctx.total}} 条记录",
            loading: "加载中...",
            noResults: "无匹配结果!",
            refresh: "刷新",
            search: "查询条件"
        ...
        templates: {
            ...
            header: "<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-12 actionBar\"><p class=\"{{css.toolbox}}\"></p><p class=\"{{css.search}}\"></p><p class=\"{{css.actions}}\"></p></div></div></div>",
            ...
            toolbox: '<div class="{{css.toolbox}}"></div>',
            search: "<div class=\"{{css.search}}\"><div class=\"input-group\"><input type=\"text\" class=\"form-control pull-right {{css.searchField}}\" placeholder=\"{{lbl.search}}\"><span class=\"input-group-btn\"><button id=\"search\" class=\"btn btn-default\" type=\"button\">搜索</button></span></div></div>",
