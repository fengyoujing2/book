/*!
 * ZUI: Standard edition - v1.5.0 - 2016-09-22
 * http://zui.sexy
...
(function ($) {
    ...
    navmenu.prototype.initSelector = function () {
        var selector = this.$.find('a[href="' + window.location.hash + '"]');
        if (selector.length > 0) {
             $('.nav-menu .active').removeClass('active');
            selector.parent().addClass('active');
            var parent = selector.parent().parent().parent('.nav-parent');
            if (parent.length>0) {
                $(parent).find('.nav-menu').slideToggle();
                $(parent).find('.icon-chevron-right').toggleClass('icon-rotate-90');
            }
        }
    }
    ...
    navmenu.prototype.initEvent = function () {
        ...
        var clickEvent = function (e) {
            if (!$(e.currentTarget).hasClass('nav-parent')) {
                $('.nav-menu .active').removeClass('active');
                $(e.currentTarget).toggleClass('active');
                if (!$(e.currentTarget).parent().hasClass('.nav-parent'))
                    isFlag++;
                return;
            } 
            if (isFlag > 0) {
            ...
            $(e.currentTarget).find('.nav-menu').slideToggle();
            $(e.currentTarget).find('.icon-chevron-right').toggleClass('icon-rotate-90');
        };

        this.$.find('.nav-menu li').unbind('click', clickEvent);
        this.$.find('.nav-menu li').on('click', clickEvent);

    };

    navmenu.prototype.SetData = function (data) {
        var menucontent = '<nav class="menu" data-toggle="menu" style="width: atuo;"><ul class="nav-menu">';
        data.forEach(function (e) {
            if (e.children && e.children.length > 0) {
                menucontent += ' <li class="nav-parent">';
                menucontent += '<a href="#"><i class="' + e.icon + '"></i>' + e.text;
                menucontent += '<i class="icon-chevron-right nav-menu-parent-fold-icon"></i></a><ul class="nav-menu hide">';

                e.children.forEach(function (child) {
                    menucontent += '<li class="'+(child.active!=undefined && child.active===true?'active':'' )+'"><a href="' + child.url + '"  ><i class="' + child.icon + '"></i> ' + child.text + '</a></li>';
                })
                menucontent += '</ul></li>';
            } else
                menucontent += '<li class="'+(e.active!=undefined && e.active===true?'active':'' )+'"><a href="' + e.url + '"><i class="' + e.icon + '"></i> ' + e.text + '</a></li>';
        });
        menucontent += '</ul> </nav>';
